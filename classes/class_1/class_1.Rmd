...
It is also possible to use multiple correlated traits. Only additional specification of residual and genetic correlation is needed. Note that the residual correlation is named phenotype correlation by now. We specify a second trait fat percentage (FP) with additive variance ($r_{A;FP}$) is $0.5\%^2$. Our additive genetic correlation ($r_{A;MKG;FP}$) to be 0.5 and the environments are assumed to be uncorrelated for now ($r_E=0$).
...
> Note: `summary()` returns the absolute values of correlations - negative values will appear as positive ones.
